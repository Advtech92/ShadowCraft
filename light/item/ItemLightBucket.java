package shadowcraft.light.item;
...
public class ItemLightBucket extends Item{
	/** field for checking if the bucket has been filled. */
	private final int isFull;

	public ItemLightBucket(final int par1, final int par2){
		super(par1);
		this.maxStackSize = 1;
		this.isFull = par2;
		...
		if (var12 == null) {
			...
			if (var12.typeOfHit == EnumMovingObjectType.TILE) {
				...
				if (this.isFull == 0) {
					...
					if (par2World.getBlockId(var13, var14, var15) == ShadowCraftLight.liquidLightStillID) {
						...
						if (--par1ItemStack.stackSize <= 0) {
							return new ItemStack(ShadowCraftLight.lightBucket);
						}
					...
					if (this.isFull < 0) {
						return new ItemStack(ShadowCraftLight.glassBucket);
					}
	...
	public boolean func_77875_a(final World par1World, final double par2, final double par4, final double par6,
		...
		if (this.isFull <= 0) {
			...
			par1World.setBlockWithNotify(par8, par9, par10, ShadowCraftLight.liquidLightStillID);
