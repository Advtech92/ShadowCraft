...
public abstract class BlockLiquidBase extends Block implements ILiquid{
	public BlockLiquidBase(final int par1, final Material par2Material){
	...
	protected int getFlowDecay(final World par1World, final int par2, final int par3, final int par4){
		return par1World.getBlockMaterial(par2, par3, par4) == this.blockMaterial ? par1World.getBlockMetadata(par2,
			par3, par4) : -1;
	}
	 ...
	 */
	protected int getEffectiveFlowDecay(final IBlockAccess par1IBlockAccess, final int par2, final int par3,
		final int par4){
	 ...
	 */
	@Override
	public boolean isBlockSolid(final IBlockAccess par1IBlockAccess, final int par2, final int par3, final int par4,
		final int par5){
		final Material var6 = par1IBlockAccess.getBlockMaterial(par2, par3, par4);
		return var6 == this.blockMaterial ? false : (par5 == 1 ? true : (var6 == Material.ice ? false : super
	 ...
	 */
	public void velocityToAddToEntity(final World par1World, final int par2, final int par3, final int par4,
		final Entity par5Entity, final Vec3 par6Vec3){
	...
	public void randomDisplayTick(final World par1World, final int par2, final int par3, final int par4,
		...
		if (this.blockMaterial == Material.water) {
			if (par5Random.nextInt(10) == 0) {
				...
				if ((var6 <= 0) || (var6 >= 8)) {
					par1World.spawnParticle("suspended", par2 + par5Random.nextFloat(), par3 + par5Random.nextFloat(),
			...
			for (var6 = 0; var6 < 0; ++var6) {
				...
				if ((par1World.getBlockMaterial(var8, par3, var9) == Material.air)
					&& (par1World.getBlockMaterial(var8, par3 - 1, var9).blocksMovement() || par1World
						.getBlockMaterial(var8, par3 - 1, var9).isLiquid())) {
		...
		if ((this.blockMaterial == Material.water) && (par5Random.nextInt(64) == 0)) {
			...
			if ((var6 > 0) && (var6 < 8)) {
				par1World.playSound(par2 + 0.5F, par3 + 0.5F, par4 + 0.5F, "liquid.water",
					(par5Random.nextFloat() * 0.25F) + 0.75F, (par5Random.nextFloat() * 1.0F) + 0.5F);
			}
		}
		...
		if ((this.blockMaterial == Material.lava) && (par1World.getBlockMaterial(par2, par3 + 1, par4) == Material.air)
			...
			if (par5Random.nextInt(100) == 0) {
				...
				par1World.playSound(var21, var22, var23, "liquid.lavapop", 0.2F + (par5Random.nextFloat() * 0.2F),
					0.9F + (par5Random.nextFloat() * 0.15F));
			}

			if (par5Random.nextInt(200) == 0) {
				par1World.playSound(par2, par3, par4, "liquid.lava", 0.2F + (par5Random.nextFloat() * 0.2F),
					0.9F + (par5Random.nextFloat() * 0.15F));
			}
		}

		if ((par5Random.nextInt(10) == 0) && par1World.doesBlockHaveSolidTopSurface(par2, par3 - 1, par4)
	 ...
	 */
	public static double getFlowDirection(final IBlockAccess par0IBlockAccess, final int par1, final int par2,
		final int par3, final Material par4Material){
		Vec3 var5 = null;

		if (par4Material == Material.water) {

			var5 = ((BlockLiquidBase) ShadowCraftShadow.liquidShadowMoving).getFlowVector(par0IBlockAccess, par1, par2,
				par3);
		}

		return (var5.xCoord == 0.0D) && (var5.zCoord == 0.0D) ? -1000.0D : Math.atan2(var5.zCoord, var5.xCoord)
			- (Math.PI / 2D);
	}
	...
	protected void triggerLavaMixEffects(final World par1World, final int par2, final int par3, final int par4){
		...
		for (int var5 = 0; var5 < 8; ++var5) {
			par1World.spawnParticle("largesmoke", par2 + Math.random(), par3 + 1.2D, par4 + Math.random(), 0.0D, 0.0D,
				0.0D);
		}
	}
}
